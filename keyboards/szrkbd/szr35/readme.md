...
Build a VIA firmware (for use with usevia.app):

  make szrkbd/szr35:via

Build a Vial firmware (for use with vial.rocks / Vial desktop):
...
For split keyboards, make sure both halves are flashed with the same target (both via, both vial, or both default).
If halves are mixed, the connected half may expose a non-matching protocol and VIA/Vial will fail to connect.
